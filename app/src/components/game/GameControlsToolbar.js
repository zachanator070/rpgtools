...
import {
	...
	RedoOutlined,
	VideoCameraOutlined,
	SelectOutlined, EnvironmentOutlined, PlusCircleOutlined, TeamOutlined
...
export const GameControlsToolbar = ({controlsMode, setControlsMode}) => {
	...
	toolTipText[ADD_MODEL_CONTROLS] = <>
		Add a model to this game
	</>;
	...
	const mouseOverText = {};
	mouseOverText[CAMERA_CONTROLS] = <>
		Move Camera, hotkey: c
	</>;
	mouseOverText[PAINT_CONTROLS] = <>
	...
	mouseOverText[ROTATE_MODEL_CONTROLS] = <>
		Rotate Model, hotkey: r
	</>;
	mouseOverText[DELETE_CONTROLS] = <>
		Delete Model, hotkey: x
	</>;
	mouseOverText[FOG_CONTROLS] = <>
		Fog of War Brush, hotkey: f
	</>;
	mouseOverText[SELECT_MODEL_CONTROLS] = <>
	...
	menu.push(...Object.keys(icons).map((mode) => {
		if(!permission[mode]){
			return null;
		}
		return <span

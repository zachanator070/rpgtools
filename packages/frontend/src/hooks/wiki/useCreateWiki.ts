...
import { WikiFolder, WikiPage } from "../../types";
...
export default function useCreateWiki(): CreateWikiResult {
	const result = useGQLMutation<WikiFolder, CreateWikiVariables>(
		CREATE_WIKI,
		{},
		{
				...
				cache.updateQuery(
					...
					(oldData = { wikisInFolder: { docs: [] } }) => ({
						...
						wikisInFolder: {
							...
							docs: [...oldData.wikisInFolder.docs, newWiki].sort((a: WikiPage, b: WikiPage) =>
								a.name < b.name ? -1 : 1,
							),
						},
					}),

...
export default function useWikisByCalendar({calendarId}: {calendarId: string}): WikisByCalendarResult {
    ...
    const variables = {worldId: world_id, type: EVENT_WIKI};
    const result = useGQLQuery<WikiPagePaginatedResult, WikisByCalendarVariables>(WIKIS_IN_FOLDER, variables);
    ...
    if(result.data) {
        result.data.docs = result.data.docs.filter((wiki: EventWiki) => wiki.calendar._id === calendarId);
    }
    return {
        ...result,
        wikis: result.data,

...
export const MoveFolderModal = ({folder, visibility, setVisibility}) => {
	...
	>
		...
		<Form
			...
			onFinish={async () => {
				await moveFolder({folderId: folder._id, parentFolderId: selectedParent});
			}}
		>
			<Form.Item
				label={'New Parent'}
				{...formItemLayout}
			>
				<SelectFolder onChange={async (value) => await setSelectedParent(value)}/>
			</Form.Item>
			<Form.Item
				{...tailLayout}
			>
				<Button type={'primary'} htmlType="submit">Move Folder</Button>
			</Form.Item>
		</Form>
	</Modal>;
};
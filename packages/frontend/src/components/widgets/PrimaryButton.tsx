...
export default function PrimaryButton({className, loading, onClick, children, disabled, submit, style, id}: PrimaryButtonProps) {
    return <Button type={'primary'} disabled={disabled || loading} className={className} loading={loading} onClick={onClick} htmlType={submit ? "submit": "button"} style={style} id={id}>
        {children}
    </Button>;
}
...
export default function AgeEditor({
	age,
	onChange = () => {},
	editable,
}: {
					...
					<span className={"margin-md-left"}>
						{editable ? (
							<NumberInput
								...
								onChange={(value) => setNumYears(value)}
								minValue={1}
							/>
						) : (
							numYears
					...
					.map((month, index) => [
						...
						<div key={`delete${index}`}>
							{editable && (
								<PrimaryDangerButton
									onClick={() => {
										setMonths(months.filter((oldMonth, oldIndex) => oldIndex !== index));
									}}
								>
									<DeleteIcon />
								</PrimaryDangerButton>
				...
				{age.daysOfTheWeek.map((dayOfTheWeek, index) => [
					<div key={`name${index}`}>
						{editable ? (
							<TextInput
								style={{ width: "15rem" }}
								defaultValue={dayOfTheWeek.name}
								onChange={(event) => {
					...
					<div key={`delete${index}`}>
						{editable ? (
							<PrimaryDangerButton
								onClick={() => {
									setDaysOfTheWeek(
										daysOfTheWeek.filter((oldMonth, oldIndex) => oldIndex !== index),
									);
								}}
							>
								<DeleteIcon />

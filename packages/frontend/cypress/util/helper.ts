...
export function seedNewServer() {
    cy.exec("npm run seed:new true >> seed.log 2>> seed.log", {log: true}).its('code').should('eq', 0);
}

export function seedMiddleEarth() {
    cy.exec("npm run seed:middle_earth true >> seed.log 2>> seed.log", {log: true}).its('code').should('eq', 0);
}

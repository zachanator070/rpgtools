...
roleSchema.methods.userCanAdmin = async function (user) {
	return (
		(await user.hasPermission(ROLE_ADMIN, this._id)) ||
		(await user.hasPermission(ROLE_ADMIN_ALL, this.world))
	);
};

roleSchema.methods.userCanWrite = async function (user) {
	return (
		(await user.hasPermission(ROLE_RW, this._id)) ||
		(await user.hasPermission(ROLE_RW_ALL, this.world))
	);
};

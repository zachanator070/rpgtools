...
export const authorizationMutations = {
	grantUserPermission: async (
		_,
		{ userId, permission, subjectId, subjectType },
		{ currentUser }
	) => {
		let newPermission = await PermissionAssignment.findOne({
			permission,
			subject: subjectId,
		}).populate("subject");
		if (!newPermission) {
			newPermission = new PermissionAssignment({
				permission,
				subject: subjectId,
				subjectType,
			}).populate("subject");
	...
	revokeUserPermission: async (
		...
		if (permissionAssignment) {
			...
			user.permissions = user.permissions.filter(
				(userPermission) => !userPermission._id.equals(permissionAssignment._id)
			);
			await user.save();
		}
	...
	grantRolePermission: async (
		_,
		{ roleId, permission, subjectId, subjectType },
		{ currentUser }
	) => {
		let newPermission = await PermissionAssignment.findOne({
			permission,
	...
	deleteRole: async (_, { roleId }, { currentUser }) => {
		...
		world.roles = world.roles.filter((otherRole) => {
			return !otherRole._id.equals(role._id);
		});
		await world.save();

	...
	const commands = [
		...
		},
		{
			...
			exec: function (args, options) {
				...
				const matches = args
					.find((arg) => arg.name === "DICE")
					.value.match(/(\d+d\d+([+-]\d+)?)+/gm);
				if (matches) {
					const rollResults = [];
					for (let match of matches) {
						...
						const roller = options.find((option) => option.name === "-q")
							? "You"
...
export const gameMutations = {
	...
	leaveGame: async (_, { gameId }, { currentUser }) => {
		...
		game.characters = game.characters.filter(
			(character) => !character.player.equals(currentUser._id)
		);
		await game.save();
	...
	addStroke: async (_, { gameId, path, type, size, color, fill, strokeId }, { currentUser }) => {
		...
		await game.save();
		await pubsub.publish(GAME_STROKE_EVENT, {
			gameId,
	...
	addFogStroke: async (_, { gameId, path, type, size, strokeId }, { currentUser }) => {
		...
		await game.save();
		await pubsub.publish(GAME_FOG_STROKE_ADDED, {
			gameId,
	...
	addModel: async (_, { gameId, modelId, wikiId, color }, { currentUser }) => {
		...
		await game.save();
		await pubsub.publish(GAME_MODEL_ADDED, {
			gameId,
	...
	setModelPosition: async (
		_,
		{ gameId, positionedModelId, x, z, lookAtX, lookAtZ },
		{ currentUser }
	) => {
		const game = await GameModel.findById(gameId);
		if (!game) {
		...
		model.wiki = await WikiPageModel.findById(model.wiki);

		await pubsub.publish(GAME_MODEL_POSITIONED, {
			gameId,
			gameModelPositioned: model,
		});
	...
	setCharacterAttributes: async (_, { gameId, str, dex, con, int, wis, cha }, { currentUser }) => {
		const game = await GameModel.findById(gameId);
		if (!game) {
			throw new Error("Game does not exist");
		}
		...
		for (let character of game.characters) {
			if (character.player.equals(currentUser._id)) {
		...
		if (str !== undefined) {
			userCharacter.str = str;
		}
		if (dex !== undefined) {
			userCharacter.dex = dex;
		}
		if (con !== undefined) {
			userCharacter.con = con;
		}
		if (int !== undefined) {
			userCharacter.int = int;
		}
		if (wis !== undefined) {
			userCharacter.wis = wis;
		}
		if (cha !== undefined) {
			userCharacter.cha = cha;
		}
		await game.save();
		return game;
	},

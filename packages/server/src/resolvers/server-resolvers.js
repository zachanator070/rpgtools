...
const getDocuments = async (model, ids) => {
  ...
  let documents = ids.filter(
    (document) => !(document instanceof mongoose.Types.ObjectId)
  );
...
const getPermissionControlledDocument = async (model, id, currentUser) => {
  if (id) {
    const document =
      id instanceof mongoose.Types.ObjectId ? await getDocument(model, id) : id;
    if (await document.userCanRead(currentUser)) {
      return document;
    }
  }
...
export const serverResolvers = {
  ...
  World: {
    ...
    rootFolder: async (world, _, { currentUser }) => {
      return await getPermissionControlledDocument(
        WikiFolder,
        world.rootFolder,
        currentUser
      );
  ...
  User: {
    ...
    currentWorld: async (user, _, { currentUser }) => {
      ...
      return getPermissionControlledDocument(
        World,
        user.currentWorld,
        currentUser
      );
  ...
  Game: {
    ...
    models: async (game, _, { currentUser }) => {
      ...
      for (let model of game.models) {
        ...
        model.wiki = await getPermissionControlledDocument(
          WikiPage,
          modelWiki,
          currentUser
        );

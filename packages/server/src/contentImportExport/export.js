...
export const addModelToArchive = async (model, archive) => {
    ...
    archive.append(Readable.from([JSON.stringify(modelObject)]), {name: modelExportedFilename});
...
export const addWikiPageToArchive = async (page, archive) => {
    ...
    archive.append(Readable.from([JSON.stringify(pageObject)]), {name: `${path}${page.constructor.modelName}.${pageObject._id}.json`});
}
...
import {ANON_USERNAME, WIKI_RW} from "@rpgtools/common/src/permission-constants";
...
describe('folder-mutations', () => {
	...
	describe('with existing world', () => {
		...
		beforeEach(async () => {
			...
			newFolder = await WikiFolder.create({name: 'new folder', world: world._id});
		...
		describe('with unauthenticated user', () => {
			test('create folder permission denied', async () => {
				const result = await mutate({mutation: CREATE_FOLDER, variables: {name: 'new folder', parentFolderId: world.rootFolder.toString()}});
				expect(result).toMatchSnapshot();
			});

			test('rename folder permission denied', async () => {
				const result = await mutate({mutation: RENAME_FOLDER, variables: {name: 'new folder', folderId: world.rootFolder.toString()}});
				expect(result).toMatchSnapshot();
			});

			test('delete folder permission denied', async () => {
				const result = await mutate({mutation: DELETE_FOLDER, variables: {folderId: newFolder._id.toString()}});
				expect(result).toMatchSnapshot({
					errors: [{
		...
		describe('with authenticated user', () => {
			beforeEach(async () => {
				currentUser = await User.findOne({username: 'tester'}).populate({path: 'roles permissions', populate: {path: 'permissions'}});
			...
			test('delete root folder', async () => {
				const result = await mutate({mutation: DELETE_FOLDER, variables: {folderId: world.rootFolder.toString()}});
				expect(result).toMatchSnapshot();
			});
		});
		...
		describe('with third user', () => {
			...
			beforeEach(async () => {
				...
				const childPage = await WikiPage.create({name: 'page', type: ARTICLE, world: world});
				newFolder.pages.push(childPage);
				await newFolder.save();
				const assignment = await PermissionAssignment.create({permission: WIKI_RW, subject: newFolder, subjectType: WIKI_FOLDER});

...
export class ContentImporter {
	...
	async importImage (properties){
		...
		for(let chunk of chunks){
			...
			if(!chunk.fileId){
				throw new Error(`Chunk ${chunk._id} references file ${chunk.fileId} but is not provided in archive`);
			}
			delete chunk._id;
	...
	async importWikiPage (path, properties){
		...
		const folderStructure = path.replace(this.getFilenameFromPath(path), '').slice(0,-1).split('/');
		...
		if(properties.contentId){
			if(!Object.keys(this.processedDocs).includes(properties.contentId)){
				throw new Error(`Wiki page ${properties.name} references content ${properties.contentId} but is not provided in archive`);
			}
		...
		if(modelName === PLACE){
			...
			if(properties.model){
				if(!this.processedDocs[properties.model]){
					throw new Error(`Wiki page ${properties.name} references model ${properties.model} but was not provided in archive`);
				}
	...
	async importModel(properties){
		...
		if(!this.processedDocs[properties.fileId]){
			throw new Error(`Model ${properties.name} references file ${properties.fileId} which was not provided in archive`);
		}
	...
	async importArchive(archiveReadStream, folder){
		...
		await new Promise((resolve, reject) => {
			...
			archiveReadStream.pipe(unzipper.Parse())
